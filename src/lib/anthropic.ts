...
const DEFAULT_PLANS: RecommendedImagePlanItem[] = [
  ...
  { type: 'lifestyle', index: 1, title: '场景图一', goal: '呈现产品最常见的使用场景，帮助用户快速理解用途和目标人群。', notes: ['允许环境和人物', '强调使用情境', '氛围服务卖点'] },
  { type: 'lifestyle', index: 2, title: '场景图二', goal: '若产品存在多样化使用场景，则展示综合或第二使用场景；若不适合多场景，也要与第一张形成明显区分。', notes: ['可做综合场景或第二场景', '产品仍是视觉焦点', '避免场景堆砌'] },
]
...
const IMAGE_TYPE_CONFIG: Record<string, { name: string; goal: string; notes: string[] }> = {
  ...
  'lifestyle-1': { name: '场景图一', goal: '呈现产品最常见的使用场景，帮助用户快速理解用途和目标人群。', notes: ['允许环境和人物', '强调使用情境', '氛围服务卖点'] },
  'lifestyle-2': { name: '场景图二', goal: '若产品存在多样化使用场景，则展示综合或第二使用场景；若不适合多场景，也要与第一张形成明显区分。', notes: ['可做综合场景或第二场景', '产品仍是视觉焦点', '避免场景堆砌'] },
...
const PROMPT_INSTRUCTIONS: Record<string, string> = {
  ...
  'lifestyle-1': '生成场景图一，聚焦最常见、最容易理解的核心使用场景，帮助用户一眼明白产品怎么用、适合谁用，画面自然可信、偏高端感，产品仍是视觉主角。如需出现任何文字，必须使用英文。',
  'lifestyle-2': '生成场景图二。若产品存在多样化使用方式，就展示另一种或综合使用场景；若不适合多场景，就换一个明显不同但同样真实的应用语境。画面层次丰富但不堆砌，产品始终是视觉焦点。如需文字必须为英文。',
...
- 如果当前生成的是场景图二，应更换使用场景、人物状态、空间语境、时间语境或功能组合，不能只是重复场景图一
