...
import { applyPaymentOrderSuccess, getPaymentOrderByOutTradeNo } from '@/lib/points'
...
async function handleNotify(request: NextRequest) {
  try {
    ...
    if ((params.pid || '').trim() !== pid) {
      return plainText('fail', 400)
    }

    if (!verifyZPaySign(params, key, sign)) {
      return plainText('fail', 400)
    }
    ...
    const moneyText = (params.money || '').trim()
    const paidAmount = Number(moneyText)
    ...
    if (Number((order.amountCents / 100).toFixed(2)) !== Number(paidAmount.toFixed(2))) {
      return plainText('fail', 400)
    }
