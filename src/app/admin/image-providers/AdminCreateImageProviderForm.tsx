...
export default function AdminCreateImageProviderForm() {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h3 className="text-base font-semibold text-slate-900">新增 Provider</h3>
          <p className="mt-1 text-sm text-slate-500">API key 只会在保存时写入密文，后台不会显示已有 key。</p>
        </div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        ...
        <input value={model} onChange={(e) => setModel(e.target.value)} className="input-field" placeholder="模型，例如 gpt-image-1" />
